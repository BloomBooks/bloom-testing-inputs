...
// Vendored from BloomDesktop .github/skills/edit-bloom-book/validateBloomBook.mjs.
// Only the jsdom-loading preamble differs: here jsdom comes from this repo's own
// package.json. If the upstream validator changes, re-vendor it and re-apply this
// preamble.
...
const loadJSDOM = () => {
    try {
        ...
        return repoRequire("jsdom").JSDOM;
    } catch (error) {
        const details = error instanceof Error ? error.message : String(error);
        console.error(
            "Failed to load jsdom. Run pnpm install at the repo root if dependencies are missing.",
        );
...
const usage = () => {
    console.log(
        "Usage: node scripts/validateBloomBook.mjs <Bloom.html-or-.htm> [more files]",
    );
};
